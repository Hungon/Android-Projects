...
public class DemoNoSQLTableOffRoad extends DemoNoSQLTableBase {
    private static final String LOG_TAG = DemoNoSQLTableOffRoad.class.getSimpleName();
    ...
    public class DemoGetWithPartitionKey extends DemoNoSQLOperationBase {
        ...
        public boolean loadItems() throws AmazonClientException {
            ...
            result = mapper.load(OffRoadDO.class, SystemManager.getUserId());
        ...
        public List<DemoNoSQLResult> getNextResultGroup() {
            ...
            results.add(new DemoNoSQLOffRoadResult(result));
    ...
    public class DemoScanWithFilter extends DemoNoSQLOperationBase {
        ...
        private PaginatedScanList<OffRoadDO> results;
        private Iterator<OffRoadDO> resultsIterator;
        ...
        public boolean loadItems() {
            ...
            results = mapper.scan(OffRoadDO.class, DemoNoSQLTableBase.getDynamoDBScanExpression(this.mAttributeName));
            if (results != null) {
                // sort process
                int count = 0;
     ...
     */
    private static List<DemoNoSQLResult> getNextResultsGroupFromIterator(final Iterator<OffRoadDO> resultsIterator) {
        if (!resultsIterator.hasNext()) {
            return null;
        }
        ...
        do {
            ...
            final OffRoadDO item = resultsIterator.next();
            // Add the item to a group of results that will be displayed later.
            resultGroup.add(new DemoNoSQLOffRoadResult(item));
            itemsRetrieved++;
    ...
    public DemoNoSQLTableOffRoad() {
        mapper = AWSMobileClient.defaultMobileClient().getDynamoDBMapper();
    }
    @Override
    public String getTableName() {
        return TABLE_SIMPLE_NAME_OFFROAD;
    }
    ...
    public void insertData(int userId,String userName,int[] records)
        ...
        final OffRoadDO item = new OffRoadDO();
    ...
    public boolean updateItems(Context context) throws AmazonClientException {
        // Use an expression names Map to avoid the potential for attribute names
        // colliding with DynamoDB reserved words.
        PaginatedScanList<OffRoadDO> results;
        ...
        for (int i = 0; i < TABLE_ATTRIBUTE_NAME_LEVEL_LIST.length; i++) {
            results = mapper.scan(OffRoadDO.class, DemoNoSQLTableBase.getDynamoDBScanExpression(TABLE_ATTRIBUTE_NAME_LEVEL_LIST[i]));
            // sort process
            int count = 0;
            int scoreList[] = new int[results.size()];
            int idList[] = new int[results.size()];
        ...
        for (int i=0;i<records.length;i++) {
            records[i] = systemManager.getTheRecords(STAGE_OFF_ROAD,LEVEL_NUMBER_LIST[i])[RECORD_TOTAL];
        }
        // get each the best record from the local storage.
        // to get aggregate score
        boolean updated = false;
        OffRoadDO update = new OffRoadDO();
    ...
    public void removeData(int userId) throws AmazonClientException {
        ...
        PaginatedScanList<OffRoadDO> results = mapper.scan(OffRoadDO.class, DemoNoSQLTableBase.getDynamoDBScanExpression(userId));

        Iterator<OffRoadDO> resultsIterator = results.iterator();
