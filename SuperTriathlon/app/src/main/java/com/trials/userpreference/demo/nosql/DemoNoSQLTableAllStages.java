...
public class DemoNoSQLTableAllStages extends DemoNoSQLTableBase {
    private static final String LOG_TAG = DemoNoSQLTableAllStages.class.getSimpleName();
    ...
    public class DemoGetWithPartitionKey extends DemoNoSQLOperationBase {
        ...
        public boolean loadItems() throws AmazonClientException {
            ...
            result = mapper.load(AllStagesDO.class, SystemManager.getUserId());
        ...
        public List<DemoNoSQLResult> getNextResultGroup() {
            ...
            results.add(new DemoNoSQLAllStagesResult(result));
    ...
    public class DemoScanWithFilter extends DemoNoSQLOperationBase {
        ...
        private PaginatedScanList<AllStagesDO> results;
        private Iterator<AllStagesDO> resultsIterator;
        ...
        public boolean loadItems() {
            ...
            results = mapper.scan(AllStagesDO.class, DemoNoSQLTableBase.getDynamoDBScanExpression(this.mAttributeName));
            if (results != null) {
                // sort process
                int count = 0;
     ...
     */
    private static List<DemoNoSQLResult> getNextResultsGroupFromIterator(final Iterator<AllStagesDO> resultsIterator) {
        if (!resultsIterator.hasNext()) {
            return null;
        }
        ...
        do {
            ...
            final AllStagesDO item = resultsIterator.next();
            // Add the item to a group of results that will be displayed later.
            resultGroup.add(new DemoNoSQLAllStagesResult(item));
            itemsRetrieved++;
    ...
    public DemoNoSQLTableAllStages() {
        mapper = AWSMobileClient.defaultMobileClient().getDynamoDBMapper();
    }

    @Override
    public String getTableName() {
        return TABLE_SIMPLE_NAME_ALLSTAGES;
    }

    @Override
    public void insertData(int userId,String userName,int[] records)
            throws AmazonClientException {
        Log.d(LOG_TAG, "Inserting Sample data.");
        final AllStagesDO item = new AllStagesDO();
    ...
    public boolean updateItems(Context context) throws AmazonClientException {
        // Use an expression names Map to avoid the potential for attribute names
        // colliding with DynamoDB reserved words.
        PaginatedScanList<AllStagesDO> results;
        ...
        int userId = SystemManager.getUserId();
        ...
        for (int stageNumber:STAGE_NUMBER_LIST) {
            for (int i = 0; i < records.length; i++) {
                records[i] += systemManager.getTheRecords(stageNumber,LEVEL_NUMBER_LIST[i])[RECORD_TOTAL];
            }
        }
        for (int i = 0; i < TABLE_ATTRIBUTE_NAME_LEVEL_LIST.length; i++) {
            results = mapper.scan(AllStagesDO.class, DemoNoSQLTableBase.getDynamoDBScanExpression(TABLE_ATTRIBUTE_NAME_LEVEL_LIST[i]));
            // sort process
            int count = 0;
            int scoreList[] = new int[results.size()];
            int idList[] = new int[results.size()];
        ...
        AllStagesDO update = new AllStagesDO();
    ...
    public void removeData(int userId) throws AmazonClientException {
        ...
        PaginatedScanList<AllStagesDO> results = mapper.scan(AllStagesDO.class, DemoNoSQLTableBase.getDynamoDBScanExpression(userId));

        Iterator<AllStagesDO> resultsIterator = results.iterator();
