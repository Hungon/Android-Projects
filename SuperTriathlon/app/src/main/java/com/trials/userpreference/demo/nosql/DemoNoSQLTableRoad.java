...
public class DemoNoSQLTableRoad extends DemoNoSQLTableBase {
    private static final String LOG_TAG = DemoNoSQLTableRoad.class.getSimpleName();
    ...
    public class DemoGetWithPartitionKey extends DemoNoSQLOperationBase {
        ...
        public boolean loadItems() throws AmazonClientException {
            ...
            result = mapper.load(RoadDO.class, SystemManager.getUserId());
        ...
        public List<DemoNoSQLResult> getNextResultGroup() {
            ...
            results.add(new DemoNoSQLRoadResult(result));
    ...
    public class DemoScanWithFilter extends DemoNoSQLOperationBase {
        ...
        private PaginatedScanList<RoadDO> results;
        private Iterator<RoadDO> resultsIterator;
        ...
        public boolean loadItems() {
            ...
            results = mapper.scan(RoadDO.class, DemoNoSQLTableBase.getDynamoDBScanExpression(this.mAttributeName));
            if (results != null) {
                // sort process
                int count = 0;
     ...
     */
    private static List<DemoNoSQLResult> getNextResultsGroupFromIterator(final Iterator<RoadDO> resultsIterator) {
        if (!resultsIterator.hasNext()) {
            return null;
        }
        ...
        do {
            ...
            final RoadDO item = resultsIterator.next();
            // Add the item to a group of results that will be displayed later.
            resultGroup.add(new DemoNoSQLRoadResult(item));
            itemsRetrieved++;
    ...
    public DemoNoSQLTableRoad() {
        mapper = AWSMobileClient.defaultMobileClient().getDynamoDBMapper();
    }

    @Override
    public String getTableName() { return TABLE_SIMPLE_NAME_ROAD; }
    ...
    public boolean updateItems(Context context) throws AmazonClientException {
        // Use an expression names Map to avoid the potential for attribute names
        // colliding with DynamoDB reserved words.
        PaginatedScanList<RoadDO> results;
        ...
        for (int i = 0; i < TABLE_ATTRIBUTE_NAME_LEVEL_LIST.length; i++) {
            results = mapper.scan(RoadDO.class, DemoNoSQLTableBase.getDynamoDBScanExpression(TABLE_ATTRIBUTE_NAME_LEVEL_LIST[i]));
            // sort process
            int count = 0;
            int scoreList[] = new int[results.size()];
            int idList[] = new int[results.size()];
        ...
        for (int i=0;i<records.length;i++) {
            records[i] = systemManager.getTheRecords(STAGE_ROAD,LEVEL_NUMBER_LIST[i])[RECORD_TOTAL];
        }
        // get each the best record from the local storage.
        // to get aggregate score
        boolean updated = false;
        RoadDO update = new RoadDO();
    ...
    public void removeData(int userId) throws AmazonClientException {
        ...
        PaginatedScanList<RoadDO> results = mapper.scan(RoadDO.class, DemoNoSQLTableBase.getDynamoDBScanExpression(userId));

        Iterator<RoadDO> resultsIterator = results.iterator();
