...
public class Collision {
    ...
    */
    public static boolean CheckTouch(int x, int y, int w, int h, float scale) {
        ...
        float before = scale * 10;
        int scaleW = (w / 10) * (int)Math.abs(before);
        int scaleH = (h / 10) * (int)Math.abs(before);
        // get touched position.
        int index = GameView.GetTouchIndex();
        Point touchPos = GameView.GetTouchedPosition(index);
        // check overlap between touched-position and object's position.
        if (x + ((w - scaleW) / 2) <= touchPos.x &&
            y + ((h - scaleH) / 2) <= touchPos.y &&
            x + ((w - scaleW) / 2) + scaleW >= touchPos.x &&
            y + ((h - scaleH) / 2) + scaleH >= touchPos.y) {
            return true;
        }
        return false;
    }
     ...
     */
    public static boolean CollisionCharacter(BaseCharacter ch1, BaseCharacter ch2) {
        // ch1's size
        ...
        if (ch1.mPos.x + Math.abs(ch1.mMoveX) < ch2.mPos.x + ch2W &&
            ch1.mPos.x + ch1W - Math.abs(ch1.mMoveX) > ch2.mPos.x &&
                ch1.mPos.y + Math.abs(ch1.mMoveY) < ch2.mPos.y + ch2H &&
                ch1.mPos.y + ch1H - Math.abs(ch1.mMoveY) > ch2.mPos.y) {
            return true;
        }
        return false;
    }
    ...
    /*
        Check to collision between two character that is Base character class.
    */
    public static boolean CollisionCharacter(BaseCharacter ch1, BaseCharacter ch2, Rect ch1Safety, Rect ch2Safety) {
        // ch1's size
        float ch1W = ch1.mSize.x * ch1.mScale;
        float ch1H = ch1.mSize.y * ch1.mScale;
        // ch2
    ...
    */
    public static boolean BroadenCollisionAreaToForward(BaseCharacter searcher, BaseCharacter target, Point detectArea) {
        // center position
     ...
     */
    public static byte NotOverlapCharacter(BaseCharacter ch1, BaseCharacter ch2, Rect safety1) {
        // return value
        byte ret = -1;
        // ch1 size
        FPoint ch1Size = new FPoint(ch1.mSize.x*ch1.mScale,ch1.mSize.y*ch1.mScale);
        // ch2 size
        FPoint ch2Size = new FPoint(ch2.mSize.x*ch2.mScale,ch2.mSize.y*ch2.mScale);
        ...
        // ch1's top
/*
        if (ch1.m_Pos.x+ch1Size.x-safety1.right < ch2.m_Pos.x &&
                ch2.m_Pos.x+ch2Size.x < ch1.m_Pos.x+safety1.left &&
                ch1.m_Pos.y+safety1.top < ch2.m_Pos.y+ch2Size.y &&
                ch2.m_Pos.y < ch1.m_Pos.y+safety1.top) {
            // modify position
            ch2.m_Pos.y = ch1.m_Pos.y-(int)ch2Size.y+safety1.top;
            // value
            ret = 1;
        }
*/
        ...
        // bottom
/*
        if (ch2.m_Pos.x < ch1.m_Pos.x+ch1Size.x-safety1.right &&
                ch1.m_Pos.x+safety1.left < ch2.m_Pos.x+ch2Size.x &&
                ch1.m_Pos.y+ch1Size.y-safety1.bottom < ch2.m_Pos.y+ch2Size.y &&
                ch2.m_Pos.y < ch1.m_Pos.y+ch1Size.y) {
            // modify position
            ch2.m_Pos.y = ch1.m_Pos.y+(int)ch1Size.y-safety1.bottom;
            // value
            ret = 3;
        }
*/
